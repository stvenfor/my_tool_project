...
Projects under the same `series` auto-inherit tags, BGM volume, and voice defaults from `modules/video-factory/work/assets/series/{series}.json`.

## Character consistency

Dialogue projects merge `character_voices` from `modules/video-factory/work/assets/characters/{key}.json` when not set in `script.json`.
