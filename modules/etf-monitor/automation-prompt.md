...
解析唯一 JSON 输出及每只候选的 portfolio_gate。若组合回撤、冷静期、`valuation_required`、`risk_reset_pending`、买入后现金低于 CNY 60,000 硬底线、两只持仓、CNY 40,000 总敞口、CNY 11,000 单笔上限、单只 CNY 20,000 或两笔上限阻止买入，必须报告 NO_ACTION。目标单笔仍为 CNY 10,000；额外 10% 仅是整手/成交容差。已有一笔时明确说明第二笔仍需人工 renewed confirmation，不能把候选当成自动许可。
