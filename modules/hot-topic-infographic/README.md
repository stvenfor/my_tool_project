...
| 步骤 | 命令 |
|------|------|
| 安装依赖 | `pip3 install -r modules/hot-topic-infographic/requirements.txt` |
...
| 产物 | 尺寸 | 路径 |
|------|------|------|
| 单张长图 | 1080×1920 | `modules/hot-topic-infographic/work/{id}/output/final.png` |
| 轮播封面 | 1080×1920 | `.../output/carousel/00-cover.png` |
| 轮播四格 | 1080×1920 ×4 | `.../carousel/01.png` ~ `04.png` |
| 布局 | JSON | `modules/hot-topic-infographic/work/{id}/layout.json` |
...
- **工作目录**：`modules/hot-topic-infographic/work/{topic_id}/upload-images/` — Codex 原图暂存、人工挑选
- **流水线输入**：`base.png`、`carousel/base-*.png` — 选定底图后手动或通过 compose 使用
- **类别预览**：`category-previews/{slug}/` — 按类别 label 的样图，与 upload-images 无关

```
modules/hot-topic-infographic/work/{topic_id}/
...
```
modules/hot-topic-infographic/work/category-previews/{storage_key}/preview.png
modules/hot-topic-infographic/work/category-previews/manifest.json
```

此目录只用于每个 label 的视觉预览，不进入定时发送队列。

话题级批量图片位于 `modules/hot-topic-infographic/work/category-topics/{storage_key}/{id}/final.png`。这里的 `manifest.json` 是定时发送的唯一状态源，使用 `label + id` 作为唯一键：已有图片默认跳过，新增 id 自动补图，发送成功后才执行 `infographic:mark-sent -- <id>`；已标记 `sent: true` 的图片之后不会再次出队。没有显式 id 的 `angles` 会生成稳定的 `{category}-{序号}-2026` id，直至满足 `min_topics`。
...
4. infographic:layout      → modules/hot-topic-infographic/work/{id}/layout.json
5. 人工审核 layout（可选）
...
node ~/.cursor/skills/douyin-image-publish/scripts/publish-imagetext.mjs \
  modules/hot-topic-infographic/work/ai-poverty-2026/copy/douyin.json
```

## 目录结构

```
modules/hot-topic-infographic/
...
modules/hot-topic-infographic/work/{id}/
