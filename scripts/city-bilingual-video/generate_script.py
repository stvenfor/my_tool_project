#!/usr/bin/env python3
"""Generate English narration and bilingual subtitles from templates."""
...
def build_subtitles(config: dict[str, Any], work_dir: Path) -> dict[str, Any]:
        ...
        segments.append(
            {
                ...
                "landmark": fill_template(str(seg.get("landmark", "")), mapping),
    ...
    duration_sec = float(config.get("duration_sec", segments[-1]["start_sec"] + segments[-1]["duration_sec"] if segments else 51))
    return {
        "hook_zh": hook_zh,
        "hook_duration_sec": float(hook.get("duration_sec", 2.5)),
        "hook_visual_keyword": fill_template(str(hook.get("visual_keyword", "")), mapping),
        "hook_landmark": fill_template(str(hook.get("landmark", mapping["city_name"])), mapping),
        "hook_phase": hook.get("phase", "day"),
        "segments": segments,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate bilingual script assets.")
