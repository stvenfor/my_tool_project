...
```bash
# Q 版全量处理（首次建议先预览）
python3 scripts/q-replace/run.py /path/to/video.mp4
...
# 从中间阶段恢复
python3 scripts/q-replace/run.py /path/to/video.mp4 --from-stage synthesize
```
...
python3 scripts/q-replace/run.py /path/to/video.mp4 \
  --profile realistic \
  --character-ref scripts/q-replace/assets/realistic-dancer/ref_front.png \
  --output-dir _q-replace/output/ref-dance-realistic \
  --preview-only \
  --frame-limit 45
```
...
```
scripts/q-replace/output/{video_stem}/
...
```bash
python3 scripts/q-replace/run.py _beat-montage/output/123.mp4 \
  --character-ref /path/to/character.png \
  --output-dir _beat-montage/output/123-q-work \
